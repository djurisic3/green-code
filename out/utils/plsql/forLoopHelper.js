...
const extractSelectStarQueries = (code) => {
    const loopPattern = /for\s+([a-zA-Z0-9_]+)\s+in\s*\(\s*(select\s+(?:[a-zA-Z0-9_]+\.\*)?\s*\*?\s+from\s+[a-zA-Z0-9_]+)/gim;
...
const replaceSelectStar = (query, columns) => {
    ...
    const columnsString = columns
        .map((col) => (identifier ? `${identifier}.${col}` : col))
        .join(", ");
    // Replace the select * or identifier.* part in the query with the column names
    return query.replace(/select\s+((?:[a-zA-Z0-9_]+\.\*)|\*)/gim, `select ${columnsString}`);
};
exports.replaceSelectStar = replaceSelectStar;
const extractLoopBody = (code, variableName, query) => {
    // Ensure the asterisk in the query is escaped properly
    const escapedQuery = query.replace(/\*/g, "\\*");
    const patternStr = `for\\s+${variableName}\\s+in\\s*\\(\\s*${escapedQuery}[\\s\\S]*?\\)\\s+loop`;
    ...
    const loopPattern = new RegExp(`for\\s+${variableName}\\s+in\\s*\\(\\s*${escapedQuery}[\\s\\S]*?\\)\\s+loop([\\s\\S]*?)end\\s+loop;`, "gim");
